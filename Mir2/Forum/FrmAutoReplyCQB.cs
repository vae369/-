...
{
    public partial class FrmAutoReplyCQB : UIPage
    {
        ...
        IForumBase forumBase = new ForumCQBHelper();
        UserInfo user = null;

        public FrmAutoReplyCQB()
        {
            InitializeComponent();
            this.Render();
        }
        ...
        private void Search()
        {
            ...
            while (IsRunSearchTopic)
            {
                ...
                try
                {
                    int count = 0, topPage = -1;
                    string url, html;
                    MatchCollection mcinfo;
                    CQ domObjects;

                    #region =============== 获取GOM端版本有多少页
                    // 取帖子总共多少页
                    url = $"{ForumConfig.CQBDomain}forum-2-1.html";
                    html = http.SendRequest(url, "", "get", "", "gbk");
                    domObjects = CQ.CreateDocument(html);
                    mcinfo = Regex.Matches(html, "共([0-9a-zA-Z\\s]+)页");

                    int.TryParse(Regex.Replace(mcinfo[0].Value, "\\D+", ""), out topPage);
                    ...
                    {
                        url = $"{ForumConfig.CQBDomain}forum-2-{i}.html";
                        html = http.SendRequest(url, "", "get", "", "gbk");
                    ...
                    url = $"{ForumConfig.CQBDomain}forum-43-1.html";
                    html = http.SendRequest(url, "", "get", "", "gbk");
                    mcinfo = Regex.Matches(html, "共([0-9a-zA-Z\\s]+)页");
                    ...
                    #region ================ 筛选GeeM2端地址和名称

                    if (topPage > 0)
                    {
                        for (int i = 1; i <= topPage; i++)
                        {
                            url = $"{ForumConfig.CQBDomain}forum-43-{i}.html";
                            html = http.SendRequest(url, "", "get", "", "gbk");
                            if (string.IsNullOrEmpty(html))
                            {
                                for (int j = 1; j <= 5; j++)
                                {
                                    ForumConfig.ShowMessage(lbMsg, $"重新获取GEE第 {i}/{topPage} 页，第{j}次...");
                                    html = http.SendRequest(url, "", "get", "", "gbk");
                                    if (!string.IsNullOrEmpty(html))
                                        break;
                                }
                            ...
                            domObjects = CQ.CreateDocument(html);
                            ForumConfig.ShowMessage(lbMsg, $"当前在取获取GEE第 {i}/{topPage} 页...");
        ...
        private void uiButton2_Click(object sender, EventArgs e)
        {
            ...
            lbuser.Items.Add(item);
            FileHelper.AppendLine(ForumConfig.CQBUserinfoPath, item);
        }
        ...
        private void uiButton4_Click(object sender, EventArgs e)
        {
            ...
            lbhuashu.Items.Add(item);
            FileHelper.AppendLine(ForumConfig.CQBHuaShuPath, item);
        }

        private void uiLinkLabel1_Click(object sender, EventArgs e)
        {
            Process.Start("IExplore", ForumConfig.CQBDomain);
        }

        private void FrmAutoReplyCQB_Load(object sender, EventArgs e)
        {
            var list1 = FileHelper.ReadTxtReturnList(ForumConfig.CQBUserinfoPath);
            foreach (var item in list1)
            {
                lbuser.Items.Add(item);
            }

            var list2 = FileHelper.ReadTxtReturnList(ForumConfig.CQBHuaShuPath);
