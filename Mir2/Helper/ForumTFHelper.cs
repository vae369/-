...
{
    public class ForumTFHelper : IForumBase
    {
        ForumConfig fc = null;
        HttpHelper http = new HttpHelper();
        Random rnd = new Random();
        string loginhash = "", formhash = "", idhash = "";
        string refer = ForumConfig.TFDomain + "forum.php";
        string setCookie = "";

        public bool AutoSingIn(UserInfo user)
        {
            string url = $"{ForumConfig.TFDomain}plugin.php?id=k_misign:sign";
            ...
            {
                url = $"{ForumConfig.TFDomain}plugin.php?id=k_misign:sign&operation=qiandao&formhash={formhash}&format=empty&inajax=1&ajaxtarget=";
                html = http.SendRequest(url, refer, "get", "", "utf-8");

                url = $"{ForumConfig.TFDomain}plugin.php?id=k_misign:sign";
                html = http.SendRequest(url, refer, "get", "", "utf-8");
        ...
        {
            string url = $"{ForumConfig.TFDomain}space-uid-{user.Uid}.html";
            string html = http.SendRequest(url, refer, "get", "", "utf-8");
            refer = url;
            CQ dom = CQ.CreateDocument(html);
        ...
        {
            // 取登录页
            string url = $"{ForumConfig.TFDomain}member.php?mod=logging&action=login&phonelogin=no&infloat=yes&handlekey=login&inajax=1&ajaxtarget=fwin_content_login";
            string html = http.SendRequest(url, refer, "get", "", "utf-8", "");
            ...
            // 取验证码页
            url = $"{ForumConfig.TFDomain}misc.php?mod=seccode&action=update&idhash={idhash}&modid=member::logging&{rnd.NextDouble()}";
        ...
        {
            string url = $"{ForumConfig.TFDomain}member.php?mod=logging&action=login&loginsubmit=yes&handlekey=login&inajax=1" + loginhash;
        ...
        public bool Logout(Topic topic)
        {
            ...
            string url = $"{ForumConfig.TFDomain}member.php?mod=logging&action=logout&formhash=" + formhash;
        ...
        public string PublishReply(Topic topic, UserInfo user, string tiename, bool IsForumReply)
        {
            ...
            do
            {
                url = $"{ForumConfig.TFDomain}forum.php?mod=post&action=reply&fid=2&tid={topic.t_Id}&infloat=yes&handlekey=reply&inajax=1&ajaxtarget=fwin_content_reply";
                html = http.SendRequest(url, refer, "get", "", "utf-8");
                formhash = CQ.Create(html).Find("input[name='formhash']").Val();
            } while (string.IsNullOrEmpty(formhash));

            url = $"{ForumConfig.TFDomain}forum.php?mod=post&infloat=yes&action=reply&fid=2&extra=&tid={topic.t_Id}&replysubmit=yes&inajax=1";
