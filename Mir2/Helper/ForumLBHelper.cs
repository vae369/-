...
{
    public class ForumLBHelper : IForumBase
    {
        HttpHelper http = new HttpHelper();
        Random rnd = new Random();
        string loginhash = "", formhash = "", idhash = "";
        string refer = ForumConfig.LBDomain + "forum.php";
        string setCookie = "";

        public bool AutoSingIn(UserInfo user)
        {
            string url = $"{ForumConfig.LBDomain}plugin.php?id=k_misign:sign";
            ...
            {
                url = $"{ForumConfig.LBDomain}plugin.php?id=k_misign:sign&operation=qiandao&formhash={formhash}&format=empty&inajax=1&ajaxtarget=";
                html = http.SendRequest(url, refer, "get", "", "utf-8");

                url = $"{ForumConfig.LBDomain}plugin.php?id=k_misign:sign";
                html = http.SendRequest(url, refer, "get", "", "utf-8");
        ...
        public UserInfo GetUserInfo(UserInfo user)
        {
            ...
            url = ForumConfig.LBDomain + mc[0].Value;
            html = http.SendRequest(url, refer, "get", "", "utf-8");
            refer = url;
            CQ dom = CQ.CreateDocument(html);
            ...
            else
            {
                ...
                user.jb = int.Parse(dom.Find("#psts .pf_l li")[3].InnerText.Substring(3).Trim());
                user.hdb = int.Parse(dom.Find("#psts .pf_l li")[4].InnerText.Substring(3).Trim());
        ...
        {
            // 取登录页
            string url = $"{ForumConfig.LBDomain}member.php?mod=logging&action=login&phonelogin=no&infloat=yes&handlekey=login&inajax=1&ajaxtarget=fwin_content_login";
            string html = http.SendRequest(url, refer, "get", "", "utf-8");
            CQ dom = CQ.CreateDocument(html);
            ...
            // 取验证码页
            url = $"{ForumConfig.LBDomain}misc.php?mod=seccode&action=update&idhash={idhash}&modid=member::logging&{rnd.NextDouble()}";
        ...
        {
            verCode = ForumConfig.LBDomain + "forum-2-1.html";

            string url = $"{ForumConfig.LBDomain}member.php?mod=logging&action=login&infloat=yes&handlekey=login&inajax=1&ajaxtarget=fwin_content_login";
            string html = http.SendRequest(url, refer, "get", "", "utf-8");
            CQ dom = CQ.CreateDocument(html);
            formhash = dom.Find("input[name='formhash']").Val();
            string action = dom.Select("form").Attr("action");

            string values = $"formhash={formhash}&referer={HttpUtility.UrlEncode(verCode)}&username={HttpUtility.UrlEncode(userName)}&password={HttpUtility.UrlEncode(passWord)}&questionid=0";
            url = $"{ForumConfig.LBDomain + action}&inajax=1";
            html = http.SendRequest(url, verCode, "post", values, "utf-8", "application/x-www-form-urlencoded");
        ...
        public bool Logout(Topic topic)
        {
            ...
            string url = $"{ForumConfig.LBDomain}member.php?mod=logging&action=logout&formhash=" + formhash;
            html = http.SendRequest(url, refer, "get", "", "utf-8");
        ...
        public string PublishReply(Topic topic, UserInfo user, string message, bool IsForumReply)
        {
            ...
            //do
            //{
            //    url = $"{ForumConfig.LBDomain}forum.php?mod=post&action=reply&fid=2&tid={topic.Id}&infloat=yes&handlekey=reply&inajax=1&ajaxtarget=fwin_content_reply";
            //    html = http.SendRequest(url, refer, "get", "", "utf-8");
            //    if (html.IndexOf("指定的主题不存在") > 0)
            //        break;
            ...
            url = $"{ForumConfig.LBDomain}forum.php?mod=post&infloat=yes&action=reply&fid=2&extra=&tid={topic.t_Id}&replysubmit=yes&inajax=1";
            url = $"{ForumConfig.LBDomain}forum.php?mod=post&action=reply&fid=2&tid={topic.t_Id}&extra=page%3D1&replysubmit=yes&infloat=yes&handlekey=fastpost&inajax=1";
            Encoding myEncoding = Encoding.GetEncoding("utf-8");

            string values = $"select={HttpUtility.UrlEncode(message)}&message={HttpUtility.UrlEncode(message)}&posttime={DateTimeHelper.GetTimeStamp(DateTime.Now)}&formhash={formhash}";
