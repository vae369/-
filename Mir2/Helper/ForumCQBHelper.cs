...
{
    public class ForumCQBHelper : IForumBase
    {
        HttpHelper http = new HttpHelper();
        Random rnd = new Random();
        string loginhash = "", formhash = "", idhash = "";
        string refer = ForumConfig.CQBDomain + "forum.php";
        string setCookie = "";

        public bool AutoSingIn(UserInfo user)
        {
            string url = $"{ForumConfig.CQBDomain}plugin.php?id=k_misign:sign";
            ...
            {
                url = $"{ForumConfig.CQBDomain}plugin.php?id=k_misign:sign&operation=qiandao&formhash={formhash}&format=empty&inajax=1&ajaxtarget=";
                html = http.SendRequest(url, refer, "get", "", "utf-8");

                url = $"{ForumConfig.CQBDomain}plugin.php?id=k_misign:sign";
                html = http.SendRequest(url, refer, "get", "", "utf-8");
        ...
        {
            string url = $"{ForumConfig.CQBDomain}forum-2-1.html";
            string html = http.SendRequest(url, refer, "get", "", "gbk");
            MatchCollection mc = Regex.Matches(html, $"{ForumConfig.CQBDomain}space-uid-([0-9a-zA-Z]+).html");
            ...
            else
            {
                ...
                user.GroupName = dom.Find(".pbm.mbm.bbda.cl a")[4] == null ? dom.Find(".pbm.mbm.bbda.cl a")[3].InnerText : dom.Find(".pbm.mbm.bbda.cl a")[4].InnerText;
        ...
        {
            // 取登录页
            string url = $"{ForumConfig.CQBDomain}member.php?mod=logging&action=login&phonelogin=no&infloat=yes&handlekey=login&inajax=1&ajaxtarget=fwin_content_login";
            string html = http.SendRequest(url, refer, "get", "", "utf-8", "");
            CQ dom = CQ.CreateDocument(html);
            ...
            // 取验证码页
            url = $"{ForumConfig.CQBDomain}misc.php?mod=seccode&action=update&idhash={idhash}&modid=member::logging&{rnd.NextDouble()}";
        ...
        {
            verCode = ForumConfig.CQBDomain + "forum.php";
            string url = $"{ForumConfig.CQBDomain}member.php?mod=logging&action=login&loginsubmit=yes&infloat=yes&lssubmit=yes&inajax=1";
            string values = string.Format("username={0}&password={1}&quickforward=yes&handlekey=ls", HttpUtility.UrlEncode(userName), HttpUtility.UrlEncode(passWord));

            string html = http.SendRequest(url, refer, "post", values, "gbk", "application/x-www-form-urlencoded");
            if (html.Contains(ForumConfig.CQBDomain) || html.Contains(verCode) || html.Contains("欢迎您回来"))
            {
                return 1;
            }
            else
        ...
        public bool Logout(Topic topic)
        {
            ...
            string url = $"{ForumConfig.CQBDomain}member.php?mod=logging&action=logout&formhash=" + formhash;
        ...
        public string PublishReply(Topic topic, UserInfo user, string message, bool IsForumReply)
        {
            ...
            do
            {
                url = $"{ForumConfig.CQBDomain}forum.php?mod=post&action=reply&fid=2&tid={topic.t_Id}&infloat=yes&handlekey=reply&inajax=1&ajaxtarget=fwin_content_reply";
                html = http.SendRequest(url, refer, "get", "", "gbk");
                formhash = CQ.Create(html).Find("input[name='formhash']").Val();
            } while (string.IsNullOrEmpty(formhash));

            url = $"{ForumConfig.CQBDomain}forum.php?mod=post&infloat=yes&action=reply&fid=2&extra=&tid={topic.t_Id}&replysubmit=yes&inajax=1";
