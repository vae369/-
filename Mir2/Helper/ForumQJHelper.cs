...
{
    public class ForumQJHelper : IForumBase
    {
        HttpHelper http = new HttpHelper();
        Random rnd = new Random();
        string loginhash = "", formhash = "", idhash = "";
        string refer = ForumConfig.QJDomain + "forum.php";
        string setCookie = "";

        public bool AutoSingIn(UserInfo user)
        {
            string url = $"{ForumConfig.QJDomain}plugin.php?id=k_misign:sign";
            ...
            {
                url = $"{ForumConfig.QJDomain}plugin.php?id=k_misign:sign&operation=qiandao&formhash={formhash}&format=empty&inajax=1&ajaxtarget=";
                html = http.SendRequest(url, refer, "get", "", "utf-8");

                url = $"{ForumConfig.QJDomain}plugin.php?id=k_misign:sign";
                html = http.SendRequest(url, refer, "get", "", "utf-8");
        ...
        {
            string url = $"{ForumConfig.QJDomain}space-uid-{user.Uid}.html";
            string html = http.SendRequest(url, refer, "get", "", "utf-8");
            refer = url;
            CQ dom = CQ.CreateDocument(html);
            ...
            user.AuthMethod = dom.Find(".pbm.mbm.bbda.cl a")[2].InnerText;
        ...
        {
            // 取登录页
            string url = $"{ForumConfig.QJDomain}member.php?mod=logging&action=login&phonelogin=no&infloat=yes&handlekey=login&inajax=1&ajaxtarget=fwin_content_login";
            string html = http.SendRequest(url, refer, "get", "", "utf-8", "");
            ...
            // 取验证码页
            url = $"{ForumConfig.QJDomain}misc.php?mod=seccode&action=update&idhash={idhash}&modid=member::logging&{rnd.NextDouble()}";
            ...
            // 图片存本地
            string path = ForumConfig.StartupPath + "\\images\\qsyzm.png";
            map.Save(path, System.Drawing.Imaging.ImageFormat.Png);

            //解析验证码
            //using (var engine = new TesseractEngine(@"./tessdata", "eng", EngineMode.Default))
            //{
            //    using (var entity = engine.Process(map))
            //    {
            //        string _checkCode = entity.GetText();
            //    }
            //}
        ...
        {
            string url = $"{ForumConfig.QJDomain}member.php?mod=logging&action=login&loginsubmit=yes&handlekey=login&inajax=1" + loginhash;
            string postData = $"formhash={formhash}&referer=https%3A%2F%2Fwww.biaomei.vip%2Fforum.php&loginfield=username&username={userName}&password={passWord}&questionid=0&answer=&seccodehash={idhash}&seccodemodid=member%3A%3Alogging&seccodeverify={verCode}";
        ...
        public bool Logout(Topic topic)
        {
            ...
            string url = $"{ForumConfig.QJDomain}member.php?mod=logging&action=logout&formhash=" + formhash;
        ...
        public string PublishReply(Topic topic, UserInfo user, string message, bool IsForumReply)
        {
            ...
            do
            {
                url = $"{ForumConfig.QJDomain}forum.php?mod=post&action=reply&fid=2&tid={topic.t_Id}&infloat=yes&handlekey=reply&inajax=1&ajaxtarget=fwin_content_reply";
                html = http.SendRequest(url, refer, "get", "", "utf-8");
            ...
            url = $"{ForumConfig.QJDomain}forum.php?mod=post&infloat=yes&action=reply&fid=2&extra=&tid={topic.t_Id}&replysubmit=yes&inajax=1";
            string post = $"formhash={formhash}&handlekey=reply&noticeauthor=&noticetrimstr=&noticeauthormsg=&usesig=0&subject=&message={HttpUtility.UrlEncode(message)}";
